...
Campus-Flow uses Cloudflare Workers as the backend:

- Cloudflare D1 database
- Cloudflare Workers (Edge Functions)
- Cloudflare Scheduled Triggers for automatic scheduling
...
The frontend must use the actual Cloudflare D1 data rather than inventing production subjects.
...
Session expires
         ↓
Scheduled triggers check active sessions
         ↓
...
The function is primarily triggered automatically by Cloudflare scheduled triggers.
...
# 8. Cloudflare Scheduled Triggers

Cloudflare Scheduled Triggers are NOT a database table.

They are Cloudflare Workers cron expressions used as a scheduler.
...
When it finds an expired active session, it triggers the HTTP request to `finalize-session`.
...
# 9. Cloudflare HTTP Requests from Scheduled Triggers

Cloudflare HTTP Requests are NOT a database table.

They allow Cloudflare Workers to make HTTP requests as part of scheduled trigger workflows.
...
Automatic trigger: Cloudflare scheduled triggers + HTTP requests
```
...
2. Treat the existing Cloudflare Workers backend as the source of truth.
